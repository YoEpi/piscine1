...
lettres_selectionnees = []  # Liste pour stocker les lettres déjà sélectionnées
...
def rejouer():
    global mot_a_deviner, mot_masque, essais, debut_chrono, lettres_alphabet, lettres_selectionnees
    mot_a_deviner = get_random_word()
    mot_masque = "_" * len(mot_a_deviner)
    essais = 0
    lettres_alphabet = [chr(i) for i in range(97, 123)]
    lettres_selectionnees = []
    maj_affichage()
    dessiner_pendu()
    debut_chrono = datetime.now()
    maj_clavier_alphabet()

# Fonction pour griser les boutons du clavier déjà sélectionnés
def griser_lettres_selectionnees():
    for lettre in lettres_selectionnees:
        for bouton in boutons_clavier:
            if bouton["text"] == lettre:
                bouton.config(state=tk.DISABLED)

# Liste des boutons du clavier
boutons_clavier = []
...
def maj_clavier_alphabet():
    for lettre in lettres_alphabet:
        ...
        boutons_clavier.append(lettre_btn)

# Fonction appelée lorsque le joueur sélectionne une lettre depuis le clavier
def lettre_selectionnee(lettre):
    if lettre not in lettres_selectionnees:
        lettres_selectionnees.append(lettre)
        verifier_lettre(lettre)
        maj_clavier_alphabet()
        griser_lettres_selectionnees()
...
rejouer_button = tk.Button(fenetre, text="Rejouer", command=rejouer, font=("Arial", 16))
